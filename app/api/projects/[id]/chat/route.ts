...
- 提示词使用英文`,
    ...
    const newPrompt = await deepseekChat(messages, { maxTokens: 1024, temperature: 0.6 });

    return NextResponse.json({ success: true, prompt: newPrompt || currentPrompt });
  } catch (err: unknown) {
    console.error("对话修改失败:", err);
    const message = err instanceof Error ? err.message : "修改失败，请稍后重试";
